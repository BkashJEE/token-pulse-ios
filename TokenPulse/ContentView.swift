    ...
    var body: some View {
        ScrollView {
            LazyVStack(spacing: 10) {
                ...
                if let hardware = snapshot.hardware { ReadinessRow(hardware: hardware) }
                HStack { Text("Active sessions").font(.headline); Spacer(); Text("Last 20 minutes").font(.caption2).foregroundStyle(.secondary) }.padding(.top, 14)
